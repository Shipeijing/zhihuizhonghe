          ...
          <ul>
            <li>
              ...
              <ul>
                ...
                <li
                  class="bi"
                  v-if="changeIndex == 8 && listByCodePs1.length >= 9"
                >
                ...
                <li
                  class="bi"
                  v-else-if="changeIndex != 8 && listByCodePs1.length >= 9"
                >
            ...
            <li v-if="listByCodePs2.length > 1">
              ...
              <ul>
                <li
                  ...
                  :class="changeactive2 == item.id ? 'active' : ''"
                  @click="open0(item.id)"
                >
                  {{ item.name }}
                </li>
            ...
            <li>
              ...
              <ul>
                <li
                  :class="jiageA === null ? 'active' : ''"
                  @click="open1(null)"
                >
                  全部
                </li>
                <li
                  v-for="(item, jiageindex) in jiage"
                  :class="jiageA == jiageindex ? 'active' : ''"
                  @click="open1(jiageindex)"
                >
                  {{ item }}
                </li>
              </ul>
            ...
            <li>
              ...
              <ul>
                ...
                <li
                  v-for="(item, hanziindex) in hanzi"
            ...
            <li>
              ...
              <ul>
                <li
                  :class="shangbiaoA === null ? 'active' : ''"
                  @click="open3(null)"
                >
                  全部
                </li>
                <li
                  v-for="(item, shangindex) in listTrademarkType"
                  :class="shangbiaoA == item.tCode ? 'active' : ''"
                  @click="open3(item.tCode)"
                >
                  {{ item.name }}
                </li>
              </ul>
              ...
              <div class="botLeft">
                <ul>
                  <li
                    ...
                    v-show="
                      item.title.length <= Tnum &&
                        item.title.length > Tnumi &&
                        Pmin <= Number(item.price) &&
                        Number(item.price) <= Pmax
                    "
...
export default {
  async asyncData () {
    let [listByCodePs1, listTrademarkType] = await Promise.all([
      Services.listByCodePs({ pCode: '10210' }),
      Services.listTrademarkType()
    ])
    return {
  ...
  beforeCreate () {
    this.$store.dispatch('setNavName', '商标交易')
  },
  data () {
    return {
      t: 1,
  ...
  methods: {
    ...
    showProductInit (data) {
      var params = {
        ...
        tCode: this.shangbiaoA,
        resourceCategoryId: this.changeactive,
        productClassId: Number(39)
      }
    ...
    open1 (id) {
      ...
      } else if (this.jiageA == 0) {
        this.Pmin = 0
        this.Pmax = 30000
      } else if (this.jiageA == 1) {
        this.Pmin = 30000
        this.Pmax = 50000
      } else if (this.jiageA == 2) {
        this.Pmin = 50000
        this.Pmax = 100000
      } else if (this.jiageA == 3) {
        this.Pmin = 100000
        this.Pmax = 200000
      } else if (this.jiageA == 4) {
        this.Pmin = 200000
    ...
    open2 (id) {
      ...
      if (this.hanziA == null) {
        this.Tnumi = 0
        this.Tnum = 1000000000000000000000000
