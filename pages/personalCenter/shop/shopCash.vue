<template>
  <div class="right">
    <div class="title">
      <p>账户余额：<span>{{indexInfo.shopBalance ? indexInfo.shopBalance:0}}</span>元</p>
      <span
        class="addBtn"
    ...
    <el-dialog
      ...
      width="30%"
    >
      <p style="margin-bottom: 20px;">当前可提现金额：{{indexInfo.shopBalance}}元；&nbsp;
        <span
          class="tipMsg"
          v-show="tipShow"
        >{{tipName}}</span></p>
...
export default {
  ...
  methods: {
    ...
    getJyList () {
      ...
      Services.coWalletDetailQuery(parmas).then(response => {
        if (response.data.code == 200) {
          self.walletList = response.data.result.walletList
